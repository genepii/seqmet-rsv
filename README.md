...
**genepii/seqmet-rsv** is a bioinformatics pipeline designed for the analysis of next generation sequencing data (NGS) Respiratory Syncitial Virus.
...
Currently it only permits to generate files for RSV. The main pipeline available on an independant repository take in charge other respiratory viruses such as SARS-CoV-2 and Influenza.
...
4. Map reads on a set of reference, permit to choose an appropriate reference for each sample (in flu case for example), and verify sequencing success of positive controls ([`bwa mem`](https://bio-bwa.sourceforge.net/bwa.shtml))
5. Map reads on each reference passing a given threshold, multiple reference can be used ([`bwa mem`](https://bio-bwa.sourceforge.net/bwa.shtml))
...
3. Clone this repository and prepare the nextflow config file and json parameters file:
   ...
   > - Build the singularity containers `nextclade`, `denovo`, `samtools`, `varcall` using the definition files found in singularity/rsv/def.
   ...
   > - Edit the json parameters template found in piperun/000000_seqmet_denovo_rsv to replace "/path/to/seqmet" by the absolute path of the cloned repository.

4. Prepare the database necessary to the worklow and indicate their path in the json parameters file:

   > db used in the pipeline are available on request (large files). A permanent link for download will be available.
   
5. Test the pipeline on the provided dataset with a single command using the json template:

   ```console
   cd seqmet/piperun
   ./launch_piperun.sh 000000_seqmet_denovo_rsv
   ```
...
6. Try on your on dataset, with different parameters:
   ...
   ./launch_piperun.sh date_seqmet_denovo_rsv
   ```
...
The pipeline is primarily designed, written, tested and maintained by Bruno Simon ([@sib0](https://github.com/sib0)), and Hadrien Regue ([@HadrienRegue](https://github.com/HadrienRegue)) from [GenEPII Sequencing Platform, Institut des Agents Infectieux, Hospices Civils de Lyon, Lyon, France](https://genepii.univ-lyon1.fr/).
