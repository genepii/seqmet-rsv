...
##Convert IUPAC ambiguous characters in a fasta file
#v0.0.1
...
for i in range(len(seq)):
    w.write('>' + seq[i][0] + '\n' + seq[i][1].replace('N','A').replace('R','A').replace('Y','C').replace('S','G').replace('W','A').replace('K','G').replace('M','A').replace('B','C').replace('D','A').replace('H','A').replace('V','A') + '\n')
