...
def vcf_parse(vcf, fields):
    ...
    for i in range(len(var)):
        fields_header = [ x.split('=')[0] for x in vcf[i].split('\t')[7].split(';') ]
...
parser.add_argument('--af_field', default='AF', help='the base')
...
tvar = vcf_parse(tvcf, args.af_field)

rvar = vcf_parse(rvcf, args.af_field)
...
if args.mode == 'raw':
    ...
    tvar_major = [ x for x in tvar if float(x[4])>=0.5 ]
    tvar_minor = [ x for x in tvar if float(x[4])<0.5 and float(x[4])>args.min_freq ]
    rvar_major = [ x for x in rvar if float(x[4])>=0.5 ]
elif args.mode == 'min' or args.mode == 'maj' or args.mode == 'coinf':
    ...
    tvar_major = [ x for x in tvar if float(x[4])>=0.5 and depth[depth_chrom.index(x[0])][1][x[1]]>args.min_depth ]
    tvar_minor = [ x for x in tvar if float(x[4])<0.5 and float(x[4])>args.min_freq and depth[depth_chrom.index(x[0])][1][x[1]]>args.min_depth ]
...
tvar_major_profile_reversed = [ [ x[0], x[1], x[3], x[2] ] for x in tvar_major ]
tvar_minor_profile = [ [ x[0], x[1], x[2], x[3] ] for x in tvar_minor ]
rvar_major_profile = [ [ x[0], x[1], x[2], x[3] ] for x in rvar_major ]
rvar_major_profile_reversed = [ [ x[0], x[1], x[3], x[2] ] for x in rvar_major ]
...
if args.exclusion:
    ...
    evar_major_profile_reversed = [ [ x[0], x[1], x[3], x[2] ] for x in evar_major ]
    expected += list_intersect(list_intersect( tvar_major_profile_reversed, rvar_major_profile_reversed , 'match', 100 ), evar_major_profile_reversed , 'missing', 100 )
    expected += list_intersect(list_intersect( tvar_major_profile_reversed, evar_major_profile_reversed , 'match', 100 ), rvar_major_profile_reversed , 'missing', 100 )
...
full_length = len(rvar)
common_length = len(common)
expected_length = len(expected)

if args.mode == 'min' or args.mode == 'maj' or args.mode == 'coinf':
    tvar_profile_full = tvar_major_profile + tvar_major_profile_reversed + tvar_minor_profile
    tvar_profile_full_af = [ x[4] for x in tvar_major ] + [ tvar_minor[tvar_minor_profile.index(x)][4] if x in tvar_minor_profile else 0 for x in tvar_major_profile_reversed ] + [ x[4] for x in tvar_minor ]
...
if args.mode == 'min' or args.mode == 'maj'or args.mode == 'coinf':
    if expected_length > 0:
        w.write(refname + "\t" + str(full_length) + "\t" + str(common) + "\t" + str(expected) + "\t" + str(common_length) + "\t" + str(expected_length) + "\t" + str(round(float(common_length)**2/float(expected_length), 3)) + "\t" + str(median) + "\t" + str(IQR) + "\n")
    else:
        w.write(refname + "\t" + str(full_length) + "\t" + str(common) + "\t" + str(expected) + "\t" + str(common_length) + "\t" + str(expected_length) + "\t0.000\t" + str(median) + "\t" + str(IQR) + "\n")
